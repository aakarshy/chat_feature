...
app.post('/register', async (req, res) => {
  ...
  if (alreadyExistingUsernameUser) {
    return res.json({ err: 'User with this name already exists' });
  }
...
app.post(
  ...
  async (req, res) => {
    ...
    const { name, userId, topic } = req.body;
...
app.post(
  ...
  async (req, res) => {
    let reqFileId = req.file.id;
    const { name, userId, receiver } = req.body;
...
mongoose
  ...
  .then(res => {
    io.on('connection', socket => {
      const { userId, name, profilePicUrl } = socket.handshake.query;
      ...
      if (socket.handshake.query.userId) {
        onlineUsers = [
          ...filteredOnlineUsers,
          { userId, name, profilePicUrl, socketId: socket.id }
        ];
      }
